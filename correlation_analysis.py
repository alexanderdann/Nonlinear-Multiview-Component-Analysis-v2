...
def CCA(view1, view2, shared_dim, pca_dim=None):
    if pca_dim is not None:
        V1 = tf.cast(PCA(n_components=pca_dim).fit_transform(view1.numpy()), dtype=tf.float32)
        V2 = tf.cast(PCA(n_components=pca_dim).fit_transform(view2.numpy()), dtype=tf.float32)
    else:
    ...
    return A, B, epsilon, omega, D[:shared_dim]
