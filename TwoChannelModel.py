...
class TwoChannelModel():
    ...
    def _generate_Gaussian_data(self, distinct_correlations=list()):
        ...
        self.plot_heatmap(Cross_Covariance)
    ...
    def plot_heatmap(matrix):
        ...
        legend = plt.imshow(matrix, cmap='Oranges')
        clrbr = fig.colorbar(legend, orientation="horizontal", pad=0.2, ax=ax)
        for t in clrbr.ax.get_xticklabels():
            t.set_fontsize(10)
        legend.set_clim(0, 1)
        clrbr.set_label(r'Correlation', fontsize=15)
        ax.set_xticks(np.arange(0, dim2, 1), labels=np.arange(1, dim2+1, 1))
        ax.set_yticks(np.arange(0, dim1, 1), labels=np.arange(1, dim2+1, 1))
        ax.tick_params(
            axis='x',
            which='both',
            bottom=False,
        ...
        for i in range(dim1):
            for j in range(dim2):
                c = np.around(matrix[j, i], 2)
                txt = ax.text(i, j, str(c), va='center', ha='center', color='black', size='x-large')
        plt.suptitle('Generated sources', size=18)
        ax.set_xlabel(r'$\mathbf{z}_{\mathrm{1}}$', fontsize=18)
        ax.set_ylabel(r'$\mathbf{z}_{\mathrm{2}}$', fontsize=18)
        plt.show()
